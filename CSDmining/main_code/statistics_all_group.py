...
path_save_data = os.path.join(path_root, 'results/statistics_all_groups/statistics_all_groups.xlsx')

# 统计
for element in list_elements:
    path_current_mol = os.path.join(path_mols_dir, element)
    test = statistics_from_crystal.StatisticsFromCrystal(list_elements)
    locals()['dict_result_{}'.format(element)], list_con_names = test.get_all_function_groups(path_current_mol, path_con)

# 判断结果保存文件夹是否存在，若不存在，则创建
if not os.path.exists(os.path.join(path_save_data, '..')):
    os.makedirs(os.path.join(path_save_data, '..'))
